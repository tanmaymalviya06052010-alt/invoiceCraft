      ...
      >
        {selectedCountry ? (
          <>
            ...
            <span className="flex-1 truncate font-medium text-surface-700">{selectedCountry.name}</span>
            {showPhoneCode && <span className="text-sm text-surface-400 font-mono">{selectedCountry.phoneCode}</span>}
          </>
        ) : (
          <span className="text-surface-400 flex-1">{placeholder}</span>
        )}
        <FiChevronDown className={`w-4 h-4 text-surface-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>

      {isOpen && (
        <div className="absolute z-50 w-full mt-2 bg-white border border-surface-100 rounded-2xl shadow-elevated-xl overflow-hidden">
            ...
            {filteredCountries.length === 0 ? (
              ...
              filteredCountries.map((country) => (
                ...
                >
                  ...
                  <span className="flex-1 text-sm text-surface-700 font-medium">{country.name}</span>
                  {showPhoneCode && <span className="text-xs text-surface-400 font-mono">{country.phoneCode}</span>}
                </button>
              ))
            )}
          </div>
